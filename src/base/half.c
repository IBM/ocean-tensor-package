...
{  uint16_t  exp;
   ...
   if (exp == OC_HALF_EXPONENT_MASK)
   {  
      /* Shift the exponent and mantissa */
      d  = ((uint32_t)(h)) << 13;

      /* Set all bits in the exponent */
      d |= OC_FLOAT_EXPONENT_MASK;
...
{  uint32_t  raw, d;
   ...
   if (exponent < (127 - 15))
   {  if ((exponent > 0) || (d != 0)) OcFPE_raise(OC_FPE_UNDERFLOW);
      return OC_HALF_ZERO;
   }
