...
# Name
NAME = libname("pyOceanDummy_cpu")

# Configuration
pyOceanDummy_CPU = Extension(NAME,
                             libraries=['ocean',
                                        'ocean_dummy_itf',
                                        'ocean_dummy_cpu'] + PY_OCEAN_SO,
                             library_dirs = [OCEAN_LIB,
                                             PY_OCEAN_LIB],
                             include_dirs = [OCEAN_INCLUDE,
                                             os.path.join(PY_OCEAN_ROOT,'pyOcean','cpu')],
                             sources=['pyOceanDummy_cpu.c'])

# Compile the extension
if (__name__ == "__main__") :
   setup(name=NAME, version="1.0", ext_modules=[pyOceanDummy_CPU])
